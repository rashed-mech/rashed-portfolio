...
// Insert useState and useEffect
const hooks = `  const [isEngaged, setIsEngaged] = useState(false);
  const [showContact, setShowContact] = useState(false);

  useEffect(() => {
    let timeout: NodeJS.Timeout;
    if (isEngaged) {
      // Shorter timeout to reveal contact faster
      timeout = setTimeout(() => setShowContact(true), 4000); 
    } else {
...
c = c.replace(/  const \[isEngaged, setIsEngaged\] = useState\(false\);\n?/g, hooks + '\n');

// Wrap CTA Reveal
const ctaRegex = /<div className="mt-6 overflow-hidden">[\s\S]*?<div className="bg-slate-900 rounded-xl p-6 md:p-8 flex flex-col md:flex-row items-center justify-between gap-6 shadow-xl border border-slate-800">[\s\S]*?<\/div>\s*<\/div>/;
const animatedCta = `<AnimatePresence>
          {showContact && (
            <motion.div
...
c = c.replace(ctaRegex, animatedCta);
