...
const SortableSkill = ({ skill, gIdx, sIdx, handleUpdateSkill, handleRemoveSkill }: any) => {
  ...
  const style = {
    ...
    ...(isDragging ? { position: 'relative', zIndex: 9999, boxShadow: '0 10px 25px -5px rgba(0, 0, 0, 0.5)' } : {}),
  };

  return (
    <div ref={setNodeRef} style={style} className="flex flex-wrap items-center gap-2 p-2.5 rounded-xl bg-slate-900/60 border border-slate-800">
      <div {...attributes} {...listeners} className="cursor-grab active:cursor-grabbing p-1 text-slate-500 hover:text-white touch-none">
...
const SortableGroup = ({ group, gIdx, handleUpdateGroupTitle, handleAddSkillToGroup, handleRemoveGroup, handleUpdateSkill, handleRemoveSkill, activeId }: any) => {
  ...
  const style = {
    ...
    ...(isDragging ? { position: 'relative', zIndex: 50, boxShadow: '0 20px 25px -5px rgba(0, 0, 0, 0.5)' } : {}),
  };

  return (
    <div ref={setNodeRef} style={style} className="p-6 rounded-2xl bg-slate-800/90 border border-slate-700/80 space-y-4">
...
export const SkillsTab: React.FC<SkillsTabProps> = ({ skillGroups, onRefresh, showToast }) => {
  ...
  const customCollisionDetection = (args: any) => {
    ...
    const isActiveGroup = String(args.active.id).startsWith('sg-');
    
    // Filter droppable containers based on what we are dragging
    const droppableContainers = args.droppableContainers.filter((container: any) => {
      const isOverGroup = String(container.id).startsWith('sg-');
      return isActiveGroup ? isOverGroup : !isOverGroup;
    });
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    const isActiveGroup = String(active.id).startsWith('sg-');
    const isOverGroup = String(over.id).startsWith('sg-');
    ...
    } else if (!isActiveGroup && !isOverGroup) {
      // Reorder skills within a group
      setGroups((items) => {
        const updated = [...items];
        
        // Find which group the active item belongs to
        const groupIdx = updated.findIndex(g => g.skills.some(s => s.id === active.id));
        if (groupIdx === -1) return updated;

        const newGroup = { ...updated[groupIdx] };
        const newSkills = [...newGroup.skills];
        
        const oldIndex = newSkills.findIndex((i: any) => i.id === active.id);
        const newIndex = newSkills.findIndex((i: any) => i.id === over.id);
        
        if (oldIndex !== -1 && newIndex !== -1) {
          newGroup.skills = arrayMove(newSkills, oldIndex, newIndex);
