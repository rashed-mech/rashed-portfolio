...
const SortableGroup = ({ group, gIdx, handleUpdateGroupTitle, handleAddSkillToGroup, handleRemoveGroup, handleUpdateSkill, handleRemoveSkill }: any) => {
...
export const SkillsTab: React.FC<SkillsTabProps> = ({ skillGroups, onRefresh, showToast }) => {
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    if (isActiveGroup && isOverGroup) {
      ...
      setGroups((items) => {
        ...
        // CRITICAL FIX: Deep copy the exact group we are modifying so React knows it changed deeply
  ...
  return (
    <div className="space-y-6 animate-fadeIn" id="admin-skills-tab">
      ...
      {/* Groups List - Only ONE DndContext here! */}
