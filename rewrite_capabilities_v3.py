        ...
        <div className="flex flex-wrap items-center gap-2 mb-8">
          <button
            ...
            className={`px-4 py-2 rounded-lg text-xs font-semibold tracking-wide transition-all cursor-pointer uppercase ${
              activeCategory === 'all'
                ? 'bg-emerald-600 text-white shadow-md shadow-emerald-600/20 border border-transparent'
          ...
          {filteredGroups.map((group, idx) => {
            ...
            return (
              <motion.div
                ...
                className="p-6 rounded-2xl bg-white border border-slate-200 hover:border-emerald-300 transition-all flex flex-col justify-between shadow-sm hover:shadow-md"
              >
                <div>
                  ...
                  <div className="flex flex-wrap gap-2.5 mt-5">
                    {group.skills.map((skill, sIdx) => (
                      ...
                      >
                        <span className={`w-1.5 h-1.5 rounded-full ${skill.highlight ? 'bg-emerald-500' : 'bg-slate-300'}`} />
